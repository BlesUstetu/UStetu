...
export default function HomePage() {
  ...
  const liveData = useMemo(() => {
    ...
    return { listingId: LISTING_ID, token: nameQuery.data ?? "USTETU", symbol: symbolQuery.data ?? "USTETU", address: token.contractAddress, seller: listing.seller, available: formatUnits(available, tokenDecimals), availableRaw: available, price: formatUnits(listing.price, paymentDecimals), priceRaw: listing.price, paymentToken: listing.paymentToken, paymentSymbol: paymentSymbolQuery.data ?? "PAYMENT", minOrderAmount: listing.minOrderAmount, maxOrderAmount: listing.maxOrderAmount, decimals: tokenDecimals, paymentDecimals, chainId: Number(token.chainId), registered: true, listingStatus: Number(listing.status) };
