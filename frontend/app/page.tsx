...
export default function HomePage() {
  ...
  const liveData = useMemo(() => {
    ...
    return { listingId: LISTING_ID, token: nameQuery.data ?? "USTETU", symbol: symbolQuery.data ?? "USTETU", address: token.contractAddress, seller: listing.seller, available: formatUnits(available, tokenDecimals), availableRaw: available, price: formatUnits(listing.price, paymentDecimals), priceRaw: listing.price, paymentToken: listing.paymentToken, paymentSymbol: paymentSymbolQuery.data ?? "PAYMENT", minOrderAmount: listing.minOrderAmount, maxOrderAmount: listing.maxOrderAmount, decimals: tokenDecimals, paymentDecimals, chainId: Number(token.chainId), status: Number(token.status), listingStatus: Number(listing.status) };
  ...
  const refreshListing = async () => { await listingQuery.refetch(); await tokenQuery.refetch(); await paymentSymbolQuery.refetch(); await paymentDecimalsQuery.refetch(); };
              ...
              <tbody>
                {hasError ? <tr><td colSpan={6} className="empty-state">{t("unableRead")}</td></tr> : isLoading ? <tr><td colSpan={6} className="empty-state">{t("readingListing")}</td></tr> : matchesSearch && liveData ? (
                  <tr className="listing-row" onClick={() => setSelected(true)}>
                    ...
                    <td>{liveData.available} {liveData.symbol}</td><td><strong>{liveData.price}</strong> {liveData.paymentSymbol}</td><td><span className="network-text">Base Sepolia</span></td>
