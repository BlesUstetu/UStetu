...
async function requestWalletTx<T>(promise: Promise<T>, action: string) {
  return withTimeout(
    ...
    `${action} belum mendapat respons dari wallet setelah 20 detik. Periksa popup wallet. Jangan klik Buy lagi sebelum memastikan transaksi ini tidak terkirim.`,
  );
}
...
export default function BuyModal(props: BuyModalProps) {
  ...
  const readOrderState = async (id: bigint) => {
    ...
    const order = await withTimeout(
      publicClient.readContract({ address: USTETU_ESCROW_ADDRESS, abi: escrowAbi, functionName: "getOrder", args: [id] }),
      RPC_REQUEST_TIMEOUT,
      "RPC order state timeout",
    );
    return order.state;
  };
  ...
  const handleBuy = async () => {
    ...
    if (chainId !== baseSepolia.id) {
      try {
        ...
        setStatusText("");
        return setErrorText(formatError(error));
      }
    }
    ...
    if (parsedAmount < minOrderAmount || parsedAmount > maxOrderAmount || parsedAmount > available) {
      return setErrorText(`Jumlah harus ${minAmount}–${maxAmount} USTETU dan tidak melebihi stok ${availableAmount} USTETU.`);
    }
    if (paymentToken.toLowerCase() !== USDC_BASE_SEPOLIA_ADDRESS.toLowerCase()) {
      return setErrorText("Listing ini menggunakan payment token yang bukan USDC Base Sepolia.");
    }
    if ((usdcBalance ?? 0n) < grossPaymentPreview) {
      return setErrorText(`Saldo USDC tidak cukup. Dibutuhkan ${formatUnits(grossPaymentPreview, USDC_DECIMALS)} USDC.`);
    }
    ...
    try {
      ...
      const createHash = await requestWalletTx(
        writeContractAsync({ address: USTETU_ESCROW_ADDRESS, abi: escrowAbi, functionName: "createOrder", args: [listingId, parsedAmount] }),
        "createOrder",
      );
      ...
      if (createdOrderId === undefined) throw new Error("OrderCreated event tidak ditemukan pada transaksi createOrder.");
      setOrderId(createdOrderId);

      let currentAllowance = allowance ?? 0n;
      if (currentAllowance < grossPayment) {
        setStep("approving");
        setStatusText("2/3 Konfirmasi approval USDC di wallet…");
        const approveHash = await requestWalletTx(
          writeContractAsync({ address: paymentToken, abi: erc20PaymentAbi, functionName: "approve", args: [USTETU_ESCROW_ADDRESS, grossPayment] }),
          "Approval USDC",
        );
      ...
      setStep("funding");
      setStatusText(`2/3 Konfirmasi pembayaran ${formatUnits(grossPayment, USDC_DECIMALS)} USDC di wallet…`);
      const fundHash = await requestWalletTx(
        writeContractAsync({ address: USTETU_ESCROW_ADDRESS, abi: escrowAbi, functionName: "fundOrder", args: [createdOrderId] }),
        "Pembayaran",
      );
      ...
      try {
        ...
        if (state !== ORDER_STATE_PAID && state !== ORDER_STATE_COMPLETED) {
          if (state === ORDER_STATE_PAYMENT_PENDING) throw new Error(`Pembayaran belum tercatat. Jangan klik Buy lagi. Tx: ${fundHash}`);
          throw fundError;
        }
      }
      ...
      if (stateAfterFunding !== ORDER_STATE_PAID) {
        throw new Error(`Order #${createdOrderId.toString()} belum PAID. Status on-chain: ${stateAfterFunding ?? "unknown"}.`);
      }

      setStep("completing");
      setStatusText("3/3 Konfirmasi penyelesaian order di wallet…");
      const completeHash = await requestWalletTx(
        writeContractAsync({ address: USTETU_ESCROW_ADDRESS, abi: escrowAbi, functionName: "completeOrder", args: [createdOrderId] }),
        "completeOrder",
      );
