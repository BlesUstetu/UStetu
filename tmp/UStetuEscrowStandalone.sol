...
/*
 * ============================================================
 * UStetu Escrow - Remix Standalone Deployment
...
library UStetuMath {
    ...
    function calculateGrossPayment(uint256 tokenAmount, uint256 unitPrice, uint8 tokenDecimals)
        internal
        pure
...
contract UStetuEscrowStandalone is ReentrancyGuard, Ownable2Step {
    ...
    function createListingAndDeposit(uint256 listingId, bytes32 tokenId, address seller, address paymentToken, uint256 price, uint256 inventoryAmount, uint256 minOrderAmount, uint256 maxOrderAmount) external nonReentrant {
        ...
        _listings[listingId] = UStetuTypes.Listing({tokenId:uint256(tokenId), seller:seller, paymentToken:paymentToken, price:price, inventoryDeposited:received, inventoryLocked:0, minOrderAmount:minOrderAmount, maxOrderAmount:maxOrderAmount, status:UStetuTypes.ListingStatus.ACTIVE, createdAt:uint64(block.timestamp), updatedAt:uint64(block.timestamp)});
    ...
    function createOrder(uint256 listingId, uint256 tokenAmount) external nonReentrant returns (uint256 orderId) {
        ...
        _orders[orderId] = UStetuTypes.Order({listingId:listingId, buyer:msg.sender, seller:listing.seller, recipient:msg.sender, token:tokenInfo.contractAddress, paymentToken:listing.paymentToken, tokenAmount:tokenAmount, unitPrice:listing.price, grossPayment:grossPayment, marketplaceFee:fee, sellerProceeds:sellerProceeds, state:UStetuTypes.OrderState.PAYMENT_PENDING, createdAt:uint64(block.timestamp), paidAt:0, completedAt:0, refundedAt:0, expiresAt:uint64(block.timestamp + PAYMENT_WINDOW), disputeId:0});
